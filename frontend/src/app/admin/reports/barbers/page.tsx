...
export default function BarberRevenueReportPage() {
  ...
  const loadReports = async () => {
    try {
      ...
      // Group by barber
      const barberMap = new Map<number, BarberReport>();

      appointments.forEach((apt: any) => {
        const barberId = apt.barber;
        const barberName = apt.barber_name;
        ...
        const report = barberMap.get(barberId)!;
      ...
      const reportsArray = Array.from(barberMap.values()).map(report => ({
        ...report,
        average_ticket: report.completed_appointments > 0
            ...
            <Grid container spacing={3} sx={{ mb: 3 }}>
              ...
              <Grid item xs={12} sm={4}>
                <Card>
                  <CardContent>
                    <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                      <TrendingUpIcon sx={{ fontSize: 32, color: 'info.main', mr: 1 }} />
                      <Typography variant="h6" color="text.secondary">
                        Concluídos
                      </Typography>
                    </Box>
                    ...
                    <TableBody>
                      {reports.map((report, index) => (
                        <TableRow key={report.barber_id}>
                          ...
                          <TableCell align="right">{formatCurrency(report.average_ticket)}</TableCell>
                          <TableCell align="right">
                            {totalRevenue > 0
